...
import io.oliverj.mammon.banking.*;
import io.oliverj.mammon.registry.BlockRegistry;
...
public class Mammon implements ModInitializer {
    public static final String MOD_ID = "mammon";
    public static final String MOD_NAME = "Mammon";
    ...
    public static Identifier id(String name) {
        return Identifier.fromNamespaceAndPath(Mammon.MOD_ID, name);
    }

    @Override
    public void onInitialize() {
        LOGGER.info("Initializing Mammon...");
...
//            }
        ...
        LOGGER.info("Registering Version Checker...");
        ...
        LOGGER.info("Registering handshakeID Receiver...");
        ...
        LOGGER.info("Registering Mammon Startup Event...");
        ServerLifecycleEvents.SERVER_STARTED.register(server -> {
            Persistence state = Persistence.getServerState(server);

            banks = state.banks;
            accounts = state.accounts;
            users = state.users;
            Mammon.LOGGER.info("Loaded {} accounts and {} banks", accounts.size(), banks.size());

            MC_SERVER = server;
        });

        LOGGER.info("Registering Mammon Shutdown Event...");
        ServerLifecycleEvents.SERVER_STOPPED.register(server -> {
            ...
            state.users = users;
            Mammon.LOGGER.info("Saved {} accounts and {} banks", accounts.size(), banks.size());
        });
    ...
    public static Component getServerErrorMessage(ErrorReason reason) {
        String desc = switch (reason) {
            case MOD_NOT_INSTALLED -> "Mammon is not installed!";
            case CLIENT_OLDER -> "You have an outdated version of the Mammon!";
            case CLIENT_NEWER -> "You have a too recent version of the Mammon!";
        };
