package io.oliverj.mammon.screen;
...
public class ATMMenu extends AbstractContainerMenu {
    ...
    public ATMMenu(int containerId, Inventory playerInventory) {
        ...
        this.accounts = Mammon.users.get(playerInventory.player.getUUID()).getAccounts().stream().map(id -> Mammon.accounts.get(id)).toList();
        this.playerInventory = playerInventory;

        acctOwnerMap = MammonClient.acctOwnerMap;
        MammonClient.acctOwnerMap = null;

        Mammon.banks.forEach((uuid, bankInfo) -> bankNameMap.put(uuid, bankInfo.getName()));
        Mammon.accounts.forEach((uuid, account) -> acctNameMap.put(uuid, account.getName()));

        Mammon.LOGGER.info("{} has {} accounts", playerInventory.player.getName(), accounts.size());
