...
public class CardItem extends Item {
    ...
    public Component getName(ItemStack stack) {
        ...
        if (Mammon.MC_SERVER.getPlayerList().getPlayer(UUID.fromString(stack.get(EconComponents.OWNER_COMPONENT_TYPE))) != null)
            return Component.translatable("item.econmod.card", Mammon.MC_SERVER.getPlayerList().getPlayer(UUID.fromString(stack.get(EconComponents.OWNER_COMPONENT_TYPE))).getName()).withStyle(style -> style.withColor(ChatFormatting.LIGHT_PURPLE));
    ...
    public @NonNull Optional<TooltipComponent> getTooltipImage(ItemStack itemStack) {
        if (itemStack.has(EconComponents.OWNER_COMPONENT_TYPE)) {
            ...
            Component ownerName = Mammon.MC_SERVER.getPlayerList().getPlayer(ownerUUID).getName();
